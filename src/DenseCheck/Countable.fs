...
type ListCountable<'T>(inner: Countable<'T>) =
    inherit Countable<'T list>()

    override _.Decode n =
        let rec decodeList n2 =
            if n2 = 0I then
                []
            else
                let head = inner.Decode((n2 - 1I) % inner.DomainSize)
                let tail = decodeList ((n2 - 1I) / inner.DomainSize)
                head :: tail
...
module Countable =
            ...
            infinite (fun n ->
                let rec decodeList n2 =
                    if n2 = 0I then
                        ...
                        let head = countable.Decode((n2 - 1I) % countable.DomainSize)
                        let tail = decodeList ((n2 - 1I) / countable.DomainSize)
                        head :: tail
    ...
    let toSet (countable: Countable<'a>) : Countable<'a Set> =
        
        let rec decodeSet (acc: 'a list) (n: bigint): 'a Set =
            if n = 0I then
                List.rev acc |> Set.ofList
            else
                let elem = countable.Decode((n - 1I) % countable.DomainSize)
                let newAcc = if List.contains elem acc then acc else elem :: acc
                decodeSet newAcc ((n - 1I) / countable.DomainSize) 

        let rec decodeFiniteSet (size: bigint) (acc: 'a list) (n: bigint) : 'a Set =
            if n = 0I then
                List.rev acc |> Set.ofList
            else
                let elem = countable.Decode((n - 1I) % size)
                let newAcc = if List.contains elem acc then acc else elem :: acc
                decodeFiniteSet size newAcc ((n - 1I) / size)

        if countable.IsInfinite then
            infinite (fun n -> decodeSet [] n)
        else
            let size = countable.DomainSize
            let newSize = size * size

            finiteCountable newSize (fun n -> decodeFiniteSet newSize [] n)
    ...
    let getPage (countable: Countable<'a>) (pageSize: bigint) (pageIndex: bigint) : 'a list =
        ...
        let endExclusive =
            if countable.IsInfinite then
                ...
                min endExclusive countable.DomainSize
    ...
    module Primitives =
        ...
        // let forInt32 = finiteCountable 4294967296I (fun n -> let n = int32 n
        //                                                      let n, r = System.Math.DivRem(n, 2)
        //                                                      if r = 0 then n else (n + 1) * -1)
        let forInt64 =
